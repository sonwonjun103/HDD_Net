...
class CA(nn.Module):
    ...
    def forward(self, x): # (B, C, D, H ,W)
        maxpool = self.maxpool(x) # (B, C, 1, 1, 1)
        attention = self.mlp(maxpool.view(x.shape[0], -1)) # (B, C)
        
        return x * attention.unsqueeze(2).unsqueeze(2).unsqueeze(2) # # (B, C, D, H ,W) (B, C, 1, 1, 1)
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        edge_output = self.edge_outc(module2_edge, edge_x4_d)
