...
#from models.cmodels.AGUnet import AGUnet
...
class Eval():
    ...
    def save_volume(self, volume, folder, model_folder, name, feature_folder=None):
        ...
        if length == 3:
            sitk.WriteImage(sitk.GetImageFromArray(volume), 
                            f"./external_resultnii\\{folder}\\{model_folder}\\{name}.nii.gz")

        elif length == 4:
            slice = volume.shape[0]
            path = f"./external_resultnii\\{folder}\\{model_folder}\\{feature_folder}"
    ...
    def evaluation(self):
        ...
        for i in range(datasize):
            ...
            folder = self.test_ct[i].split('\\')[4]

            model_folder = f"{self.args.model}_{self.args.filename}"  
            
            if os.path.exists(f"D:\\HIPPO\\FINALCODE\\external_resultnii\\{folder}\\{model_folder}") == 1:
                print(f"{i+1} {folder} exists!")
                continue                                                                 

            os.makedirs(f"D:\\HIPPO\\FINALCODE\\external_resultnii\\{folder}", exist_ok=True)
            os.makedirs(f"D:\\HIPPO\\FINALCODE\\external_resultnii\\{folder}\\{model_folder}", exist_ok=True)
...
if __name__=='__main__':
    ...
    path = f"D:\\HIPPO\\EXTERNAL\\deface\\"
    #e = ['22', '33', '43', '48', '14', '44', '45']

    for folder in os.listdir(path):
        folder_path = os.path.join(path, folder)
        for file in glob.glob(folder_path + "/*.nii"):
            filename = file.split('\\')[5]
            if filename.startswith('hippovolume'):
                hippo.append(file)
            elif filename.startswith('rct'):
                ct.append(file)

    print(len(hippo), len(ct))

    test_ct, test_hippo = ct, hippo
